...
from scripts.project.analyze_c2_c5_evaluation import decision_classification


def accumulation(c2_modes=100, c5_modes=106, c2_correct=500, c5_correct=490):
    return {
        "c2_unlikeliness_2": {
            "128": {"theorems": 10, "observed_prefix": {"correct_mode_coverage": c2_modes, "correct": c2_correct}}
        },
        "c5_reward_reject_restart": {
            "128": {"theorems": 10, "observed_prefix": {"correct_mode_coverage": c5_modes, "correct": c5_correct}}
        },
    }


def representations(delta=0.01):
    return {name: {"relative_delta_c5_minus_c2": delta} for name in (
        "first_head", "first_two_heads", "unordered_head_set", "head_multiset",
        "ordered_head_sequence", "exact_proof",
    )}


class C2C5EvaluationTests(unittest.TestCase):
    def test_material_support_requires_representation_robustness(self):
        result = decision_classification(
            accumulation(), {"16": {"relative_delta_c5_minus_c2": 0.06}}, representations()
        )
        self.assertEqual(
        ...
        mixed["exact_proof"]["relative_delta_c5_minus_c2"] = -0.01
        result = decision_classification(
            accumulation(), {"16": {"relative_delta_c5_minus_c2": 0.06}}, mixed
        )
