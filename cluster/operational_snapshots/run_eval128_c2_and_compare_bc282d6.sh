#!/usr/bin/env bash
# Evaluate finalized C2 at pass@128, then execute the frozen C2/C3 panels.
set -euo pipefail
ulimit -c 0
...
C2_TRAIN_RUN="${BUNDLE_ROOT}/runs/c2-unlikeliness-2-full-20260909-seed42-bc282d6-h100-workers64"
...
RUN_DIR="${BUNDLE_ROOT}/runs/eval128-c2-unlikeliness-2-20260909-seed42-bc282d6-workers16"
MODEL_PATH="${C2_TRAIN_RUN}/artifacts/actor/global_step_604"
...
TRAINING_RESULT="${REPOSITORY}/results/c2_unlikeliness_vs_c3_training_seed42.json"
EVALUATION_RESULT="${REPOSITORY}/results/c2_unlikeliness_vs_c3_seed42_pass128.json"
NUM_SAMPLES=128
...
if [[ ! -f "${RUN_DIR}/RUN_METADATA.md" || ! -d "${MODEL_PATH}" ]]; then
  echo "Missing fresh C2 evaluation directory or finalized C2 model." >&2
  exit 2
fi
if [[ -e "${RUN_DIR}/artifacts/proofs" || -e "${RUN_DIR}/metrics.json" || -e "${RUN_DIR}/run.log" ]]; then
  echo "Refusing a reused C2 evaluation directory." >&2
  exit 2
fi
if [[ -e "${TRAINING_RESULT}" || -e "${EVALUATION_RESULT}" ]]; then
  echo "Refusing to overwrite an existing registered C2 comparison." >&2
  exit 2
fi
...
EXEC_ROOT="${LOCAL_ROOT}/restriction-eval-c2-${SLURM_JOB_ID}-workers16"
VERIFIER_ROOT="/tmp/memoozd-rrl-verifier-eval-c2-${SLURM_JOB_ID}-workers16"
RUNTIME_HOME="${LOCAL_ROOT}/restriction-home-eval-c2-${SLURM_JOB_ID}-workers16"
RAY_TMP="/dev/shm/r${SLURM_JOB_ID}c2e"
if [[ -e "${EXEC_ROOT}" || -e "${VERIFIER_ROOT}" || -e "${RUNTIME_HOME}" || -e "${RAY_TMP}" ]]; then
  echo "Refusing to reuse node-local C2 evaluation state." >&2
  exit 2
fi
...
PYTHONPATH="${EXEC_ROOT}:${VERIFIER_SOURCE}" \
  ...
  "${RUN_DIR}" --condition c2_unlikeliness_2 --num-samples "${NUM_SAMPLES}" \
...
print(
    f"validated c2_unlikeliness_2 pass@128: correct={metrics['correct']}, "
...
PYTHONPATH="${EXEC_ROOT}:${VERIFIER_SOURCE}" \
  "${VENV_SOURCE}/bin/python" \
  "${EXEC_ROOT}/scripts/project/analyze_c2_unlikeliness.py" \
...
PYTHONPATH="${EXEC_ROOT}:${VERIFIER_SOURCE}" \
  "${VENV_SOURCE}/bin/python" \
  "${EXEC_ROOT}/scripts/project/analyze_c2_c3_evaluation.py" \
  --c0-run "${C0_EVAL}" \
  --c2-run "${RUN_DIR}" \
  --c3-run "${C3_EVAL}" \
  --output "${EVALUATION_RESULT}"

"${VENV_SOURCE}/bin/python" - "${TRAINING_RESULT}" "${EVALUATION_RESULT}" <<'PY'
import json
import sys
from pathlib import Path
...
assert training["analysis"] == "c2-soft-unlikeliness-versus-c3-hard-blocking-training-v1"
assert evaluation["analysis"] == "c2-soft-unlikeliness-versus-c3-hard-blocking-pass128-v1"
print("training_decision=" + training["registered_decision_rule"]["classification"])
print("evaluation_decision=" + evaluation["registered_decision_rule"]["classification"])
PY

echo "C2 pass@128 and both frozen C2/C3 analyses completed."
