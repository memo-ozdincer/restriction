...
D-101 admits C5 retry 4 through its exact step-20 gate under conflicting
runtime projections. Nineteen updates reconcile all 7,712 optimized proofs
with zero residual: 399 blocked correct, 4,948 alternative correct, and 2,365
incorrect, with weighted advantages -0.671/+0.537/-1.010. All 420 physical
blocks were reward-rejected; one all-blocked prompt containing 21 proofs was
correctly omitted from optimization. Timed cost is 2,794.836 seconds. Direct
primary-adjusted and uniform projections remain inside at about 23.43 and
23.52 hours, while the volatile early C3-ratio estimate is 24.87 hours. The
same conflict improved materially by step 40 in retry 2, so the unchanged run
continues to an exact step-40 gate with no partial performance claim.
...
D-104 permanently excludes C5 retry 4 at its exact step-40 runtime gate.
Thirty-eight optimizer updates
...
from optimization. Timed cost is 6,239.379 seconds. Uniform and exact-C3
projections are about 26.2 and 25.9 hours, and the run is already slower at
this gate than retries 2 and 3, both of which subsequently failed exact
step-80 feasibility. Although non-tail steps are healthy and a direct
primary-prefix estimate remains narrowly inside, that estimator was already
optimistic on the two closer predecessors. Job `871725` and zero-runtime
analysis `871726` were canceled; step 41 flushed while the replacement was
being staged but remains ineligible. Pristine retry-5 job `872448` excludes
the three nodes rejected by pre-result gates (`trig0031`, `trig0033`, and
`trig0058`); frozen analysis `872449` is bound by `afterok:872448`. Duplicate
