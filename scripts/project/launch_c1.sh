#!/usr/bin/env bash
# Run the registered full C1 GRPO-Default condition.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
RUN_DIR_INPUT="${1:?usage: launch_c1.sh /absolute/path/to/fresh-c1-run}"
RUN_DIR="$(cd "${RUN_DIR_INPUT}" && pwd)"
if [[ ! -f "${RUN_DIR}/RUN_METADATA.md" || -e "${RUN_DIR}/artifacts/actor" ]]; then
  echo "refusing unprepared or reused C1 directory: ${RUN_DIR}" >&2
  exit 2
fi
...
exec python -m verl.trainer.main_lean \
  ...
  trainer.save_freq=604 trainer.save_limit=1 trainer.save_proof_freq=604 \
  trainer.total_epochs=1 trainer.train_batch_size=256 trainer.dynamic_update=True \
  +trainer.resume=False \
  lean.prompt_key=deepseek-prover lean.num_samples=32 lean.problem_batch_size=16 \
  lean.rejection_sampling=False lean.advantage_threshold=True lean.max_workers=64 \
  +lean.penalize_extra_text=True +lean.rank_penalty=0.0 \
