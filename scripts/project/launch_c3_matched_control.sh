#!/usr/bin/env bash
# Run the full no-blocking control matched to C3 optimizer settings.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
RUN_DIR_INPUT="${1:?usage: launch_c3_matched_control.sh /absolute/path/to/fresh-control-run}"
RUN_DIR="$(cd "${RUN_DIR_INPUT}" && pwd)"
if [[ ! -f "${RUN_DIR}/RUN_METADATA.md" || -e "${RUN_DIR}/block_archive.json" || -e "${RUN_DIR}/artifacts/actor" ]]; then
  echo "refusing unprepared, archive-bearing, or reused matched-control directory: ${RUN_DIR}" >&2
  exit 2
fi
