#!/usr/bin/env bash
# Run one unchanged GRPO-Default update on a prepared 16-theorem smoke slice.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
RUN_DIR_INPUT="${1:?usage: launch_c1_smoke.sh /absolute/path/to/fresh-c1-smoke-run}"
RUN_DIR="$(cd "${RUN_DIR_INPUT}" && pwd)"
if [[ ! -f "${RUN_DIR}/RUN_METADATA.md" || -e "${RUN_DIR}/artifacts/actor" ]]; then
  echo "refusing unprepared or reused C1 smoke directory: ${RUN_DIR}" >&2
  exit 2
fi
