#!/usr/bin/env bash
# Generate and Lean-verify the fixed 32-proposal registered evaluation.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
RUN_DIR_INPUT="${1:?usage: launch_lean_evaluation.sh RUN_DIR MODEL_PATH}"
MODEL_PATH="${2:?usage: launch_lean_evaluation.sh RUN_DIR MODEL_PATH}"
