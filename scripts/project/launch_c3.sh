#!/usr/bin/env bash
# Run the registered full C3 HardBlock-Restart condition.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
RUN_DIR_INPUT="${1:?usage: launch_c3.sh /absolute/path/to/fresh-c3-run}"
RUN_DIR="$(cd "${RUN_DIR_INPUT}" && pwd)"
if [[ ! -f "${RUN_DIR}/RUN_METADATA.md" || ! -f "${RUN_DIR}/block_archive.json" || -e "${RUN_DIR}/artifacts/actor" ]]; then
  echo "refusing unprepared or reused C3 directory: ${RUN_DIR}" >&2
  exit 2
fi
